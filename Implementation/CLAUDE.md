...
| `my_optimizer.py`      | **The contest submission file.** | Wraps C++ binary as Python subclass |
| `Makefile`             | `static` / `sync` / `eval` / `validate` / `viz` | 自動化指令 |
| `tools/run_smoke_test.sh` | **自動化煙霧測試** | 比對 C++ 與 Python Cost 是否一致 |
| `tools/verify_solution.py` | Pure-Python v9 cost reimpl. | 輔助工具：數值校準用 |
| `benchmarks/`          | toy.txt / toy.sol | 煙霧測試測資 |
| `logs/activity_log.md` | **開發日誌** | 紀錄進度與變更，維持 AI 記憶 |
| `plots/`               | 擺放結果圖 | 由 `make viz` 產生 |
| `CLAUDE.md`            | This file | 內部 handover 筆記 |
...
```mermaid
graph TD
    A[C++ Source /src] -->|make| B[floorplanner binary]
    B -->|Wrapper| C[my_optimizer.py]
    C -->|API| D[Official Evaluation Script]
    D -->|Run 100 Cases| E[eval_results/ .txt + .sol]
    E -->|make viz| F[plots/ Layout PNGs]
    F -->|Analysis| G[Optimization Loop]
    G --> A
```
...
bash tools/run_smoke_test.sh  # 自動化比對 C++ 與 Python Cost
make sync        # 同步檔案至官方目錄
make validate    # 執行官方格式驗證
make eval        # 執行 100 cases 官方評估
make viz         # 執行視覺化並抓回 plots/
make clean
```
