...
1. **使用 `make validate` 驗證輸出格式。** 這是最常見的失敗原因，請確保 `my_optimizer.py` 輸出的格式完全符合官方要求。
2. **執行自動化煙霧測試 `bash tools/run_smoke_test.sh`。** 確保 C++ 核心與 Python 版本的 Cost 計算結果誤差小於 0.001。
3. **加強幾何優化 (Geometry-refinement Pass)**。
4. **調整每案時間預算 (Per-instance time budget)**。使用 `FLOORPLANNER_TIME` 環境變數。
5. **在 100 個驗證集上執行 `make eval`。** 調整 SA 參數（P_AR, P_MIB, P_FIX 等）。
